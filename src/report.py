...
import pandas as pd 
from .config import REPORTS_DIR, DASHBOARD_SUMMARY_CSV
...
def generate_markdown_report(df_recent:pd.DataFrame, stats: dict, ai_summary:str) -> str:
    ...
    ensure_reports_dir()
    def save_dashboard_summary(stats: dict, ai_summary: str):
        now = datetime.now()
        ...
        row = {
            ...
            "ai_summary_fr": ai_summary if ai_summary else ""
        }

        new_df = pd.DataFrame([row])

        if os.path.exists(DASHBOARD_SUMMARY_CSV):
            old_df = pd.read_csv(DASHBOARD_SUMMARY_CSV)
            final_df = pd.concat([old_df, new_df], ignore_index=True)
        else:
            final_df = new_df

        final_df.to_csv(DASHBOARD_SUMMARY_CSV, index=False, encoding="utf-8")
        print(f"Résumé dashboard sauvegardé : {DASHBOARD_SUMMARY_CSV}")
    now = datetime.now()
    ...
    lines.append("## 1. Statistiques génerales\n")
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    save_dashboard_summary(stats, ai_summary)

...
SOURCE_COUNTRY = {
    "ZDNet actualites": "France",
    "The Tech Buzz - Latest Articles": "USA",
    "Analytics Vidhya": "India",
    "Intelligence artificielle : Toute l’actualité sur Le Monde.fr.": "France",
    "AI News & Artificial Intelligence | TechCrunch": "USA",
    "AI | The Verge": "USA"
}
...
def parse_feed(url: str):
    ...
    print(f"\n--- TEST FLUX ---")
    print(f"URL : {url}")
    print(f"Titre du flux : {feed.feed.get('title', 'Aucun titre')}")
    print(f"Nombre d'entries : {len(feed.entries)}")
    print(f"Bozo : {feed.bozo}")
    if feed.bozo:
        print(f"Erreur détectée : {feed.bozo_exception}")

    # Ignorer les flux invalides ou vides
    if feed.bozo or not feed.entries:
        print(f"Flux ignoré : {url}")
        return []
...
def fetch_all_feeds() -> pd.DataFrame:
    ...
    df["country"] = df["source"].map(SOURCE_COUNTRY).fillna("Unknown")
    if not df.empty:
...
def save_raw_news(df: pd.DataFrame):
    ...
    if os.path.exists(RAW_NEWS_CSV):
        old_df = pd.read_csv(RAW_NEWS_CSV)
        df = pd.concat([old_df, df], ignore_index=True)

    # nettoyer la source
    df["source"] = df["source"].astype(str).str.strip()

    # recalculer country pour toutes les lignes
    df["country"] = df["source"].map(SOURCE_COUNTRY).fillna("Unknown")
    ...
    print(f"{len(df)} articles sauvegardés dans {RAW_NEWS_CSV}")
def run_fetch():
    ...
    if df.empty:
        ...
        print(df[["source", "country", "title"]].head(10))
        print(df["source"].unique())
        print(df.columns.tolist())
        print(df[["source", "country"]].head(10).to_string(index=False))
        save_raw_news(df)
